...
class DiskScanner
  ...
  def self.real_filepath(path)
    levels = path.split("\\") # Assumes path is sanitized
    case_corrected_path = ""
    case_corrected_path << levels[0].upcase + "\\" # Drive root is always uppercase
    levels_corrected = 1
    ...
    until levels.length == levels_corrected
      ...
      Dir.entries(case_corrected_path).each do |obj|
        ...
        # If exiting loop without finding anything that matches, this is set.
        # Signals that new folders need to be created, so scanning not required.
        found = false
      end
    end
    return case_corrected_path[0...-1] # last character is "\\", which is redundant
  end
end
